...
export const Participants = () => {
  ...
  const handleRequestEvent = (event) => {
    if (!requestedEvents.find(e => e.id === event.id)) {
      setRequestedEvents(prev => [...prev, event]);
      alert(`You have successfully requested to pre-register for "${event.title}".`);
    } else {
      alert(`You have already requested to pre-register for "${event.title}".`);
    }
  };
    ...
    <section className="min-h-screen bg-white/95 p-8 flex flex-col items-center pt-20">
      <div className="w-full max-w-4xl mb-12">
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden flex flex-col md:flex-row">
          ...
          <div className="p-6 flex flex-col justify-center">
            ...
            <button
              onClick={() => handleRequestEvent(latestEvent)}
        ...
        <ul className="space-y-6">
          {upcomingEvents.map(event => (
            <li key={event.id} className="bg-white rounded-lg shadow-md p-6 flex flex-col md:flex-row md:justify-between md:items-center">
              ...
              <button
                onClick={() => handleRequestEvent(event)}
                className="mt-4 md:mt-0 px-4 py-2 bg-blue-900 text-white rounded hover:bg-blue-800 transition"
              >
              Pre-register
              </button>
            </li>
      ...
      {requestedEvents.length > 0 && (
        <div className="w-full max-w-4xl mt-12">
          <h2 className="text-3xl font-semibold mb-4 text-blue-900">Your Requested Events</h2>
          <ul className="space-y-4">
            {requestedEvents.map(event => (
              <li key={event.id} className="bg-green-100 rounded-lg shadow-md p-4">
